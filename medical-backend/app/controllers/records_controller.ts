...
import { createValidator, deleteValidator, modifyValidator } from '#validators/records'
...
export default class RecordsController {
    async create({ request, response }: HttpContext) {
        ...
        const payload_more = {
            ...
            age: payload.data.age ? payload.data.age : null, 
            chest_pain: payload.data.chest_pain ? payload.data.chest_pain : null,
            restingBP: payload.data.restingBP ? payload.data.restingBP : null,
            serum_cholestrol: payload.data.serum_cholestrol ? payload.data.serum_cholestrol : null,
            fasting_blood_sugar: payload.data.fasting_blood_sugar ? payload.data.fasting_blood_sugar : null,            
            resting_electro_records: payload.data.resting_electro_records ? payload.data.resting_electro_records : null,
            max_heart_rate: payload.data.max_heart_rate ? payload.data.max_heart_rate : null,
            exercise_angia: payload.data.exercise_angia ? payload.data.exercise_angia : null,
            old_peak: payload.data.oldpeak ? payload.data.oldpeak : null,
            slope: payload.data.slope ? payload.data.slope : null,
            no_major_vessels: payload.data.no_major_vessels ? payload.data.no_major_vessels : null,
            medicalCheckDate: DateTime.fromFormat(payload.medical_record_date, 'dd/MM/yyyy').toJSDate(),
        }
    ...
    async modify({ request, response }: HttpContext) {
        ...
        const payload_more = {
            ...
            medicalCheckDate: payload.modifiedData.medical_record_date ? DateTime.fromFormat(payload.modifiedData.medical_record_date, 'dd/MM/yyyy').toJSDate() : record.medicalCheckDate,
        }
        console.log(payload_more)
